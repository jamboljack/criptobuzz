...
--
-- Host: 127.0.0.1
-- Generation Time: May 01, 2018 at 12:10 PM
...
CREATE TABLE `cripto_article` (
  ...
  `article_recomend` int(1) NOT NULL DEFAULT '1' COMMENT '1=No, 2=Yes',
  `article_tags` varchar(100) DEFAULT NULL COMMENT 'Tag',
...
INSERT INTO `cripto_contact` (`contact_id`, `contact_name`, `contact_address`, `contact_phone`, `contact_email`, `contact_web`, `contact_image`, `contact_update`) VALUES
(1, 'CRIPTOBUZZ.TODAY', 'HOUSE: 34/A, ROAD: 3/B NEW YORK, USA', '+921-3245-32', 'info@cripto.buzz.com', 'http://criptobuzz.sewamobilsentani.com', 'Contact_criptobuzztoday_1524061136.jpg', '2018-04-30 20:37:26');
...
CREATE TABLE `cripto_information` (
  ...
  `information_read` int(10) NOT NULL DEFAULT '0',
...
(1, 'criptobuzz-today', 'Arrived compass prepare an on as. Reasonable particular on my it in sympathize. Size now easy eat hand how. Unwilling he departure elsewhere dejection at. Heart large seems may purse means few blind.', 'bitcoin, criptobuzz', 'Jama&#039; Rochmad Muttaqin', 'Jama&#039; Rochmad Muttaqin', 'index, follow', 'index, follow', '2018-04-23 22:10:57');

-- --------------------------------------------------------

--
-- Table structure for table `cripto_reply`
--

CREATE TABLE `cripto_reply` (
  `reply_id` int(10) NOT NULL,
  `comment_id` int(10) NOT NULL,
  `user_username` varchar(30) NOT NULL,
  `reply_desc` text NOT NULL,
  `reply_post` datetime NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `cripto_users` (
  ...
  `user_key_reg` text,
  `user_level` enum('Admin','Operator','Member') NOT NULL DEFAULT 'Operator' COMMENT 'Level Admin',
...
INSERT INTO `cripto_users` (`user_username`, `user_password`, `user_name`, `user_mobile`, `user_email`, `user_desc`, `user_status`, `user_avatar`, `user_key_reg`, `user_level`, `user_date_create`, `user_date_update`) VALUES
('admin', '7712fcffc21bb7215f58035ab4506a5873c4af3c', 'Administrator', '085640969727', 'jama.muttaqin@gmail.com', 'Saya hanya seorang Tukang Ketik di Dunia Maya yang memberikan Informasi kepada Teman-Teman Semua. Semoga Bermanfaat.', 'Aktif', 'Avatar_admin_1516902685.jpg', NULL, 'Admin', '2018-04-30 20:42:50', '2018-01-29 20:38:51'),
('ambar', 'c7e34148827d7580828f6b6a905420c3860ff779', 'Ambar Setiyani', '08172677366', 'no-reply@sewamobilsentani.com', NULL, 'Aktif', NULL, '06dbbe9ed152e59f871d1debce3b4b55', 'Member', '2018-04-30 20:57:03', '2018-04-30 21:00:03'),
('mamad', 'f30e8a510d308f0a58ce3624d57256da630c73e5', 'Mamad', '085640969727', 'criptobuzz@sewamobilsentani.com', NULL, 'Aktif', NULL, 'd78e963a6207c8ec0c70a15cdf8aab7c', 'Member', '2018-04-30 20:51:57', '2018-04-30 21:00:29');
...
,`article_recomend` int(1)
,`article_tags` varchar(100)
...
-- (See below for the actual view)
--
CREATE TABLE `v_information` (
`information_id` int(10)
,`information_title` varchar(100)
,`information_seo` text
,`information_subtitle` varchar(100)
,`information_desc` text
,`information_icon` varchar(100)
,`information_read` int(10)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_comment`  AS  (select `c`.`comment_id` AS `comment_id`,`c`.`article_id` AS `article_id`,`c`.`user_username` AS `user_username`,`c`.`comment_desc` AS `comment_desc`,`c`.`comment_post` AS `comment_post`,`a`.`article_seo` AS `article_seo`,`u`.`user_name` AS `user_name`,`u`.`user_avatar` AS `user_avatar` from ((`cripto_comment` `c` join `cripto_users` `u` on((`c`.`user_username` = `u`.`user_username`))) join `cripto_article` `a` on((`c`.`article_id` = `a`.`article_id`)))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_information`  AS  (select `cripto_information`.`information_id` AS `information_id`,`cripto_information`.`information_title` AS `information_title`,`cripto_information`.`information_seo` AS `information_seo`,`cripto_information`.`information_subtitle` AS `information_subtitle`,`cripto_information`.`information_desc` AS `information_desc`,`cripto_information`.`information_icon` AS `information_icon`,`cripto_information`.`information_read` AS `information_read`,`cripto_information`.`information_update` AS `information_update` from `cripto_information` where (`cripto_information`.`information_id` <> 1)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_reply`  AS  (select `r`.`reply_id` AS `reply_id`,`r`.`comment_id` AS `comment_id`,`r`.`user_username` AS `user_username`,`r`.`reply_desc` AS `reply_desc`,`r`.`reply_post` AS `reply_post`,`u`.`user_name` AS `user_name`,`u`.`user_avatar` AS `user_avatar` from ((`cripto_reply` `r` join `cripto_comment` `c` on((`r`.`comment_id` = `c`.`comment_id`))) join `cripto_users` `u` on((`c`.`user_username` = `u`.`user_username`)))) ;
...
--
-- Indexes for table `cripto_banner`
--
ALTER TABLE `cripto_banner`
...
--
-- Indexes for table `cripto_reply`
--
ALTER TABLE `cripto_reply`
...
--
-- AUTO_INCREMENT for table `cripto_banner`
--
ALTER TABLE `cripto_banner`
...
--
-- AUTO_INCREMENT for table `cripto_reply`
--
ALTER TABLE `cripto_reply`
...
--
-- Constraints for table `cripto_reply`
--
ALTER TABLE `cripto_reply`
  ADD CONSTRAINT `cripto_reply_ibfk_1` FOREIGN KEY (`comment_id`) REFERENCES `cripto_comment` (`comment_id`),
  ADD CONSTRAINT `cripto_reply_ibfk_2` FOREIGN KEY (`user_username`) REFERENCES `cripto_users` (`user_username`);
COMMIT;
